...
function get_branch_number(v0, y_init, xs, num_rays, K; prefix = "fermi_dec", force = false, a, dot_radius, softness, θ_range) 
    d = @dict(v0, y_init, num_rays, xs, K, a, dot_radius, softness, θ_range)  
...
v0 = 0.1; dt = 0.01; num_rays = 200000; n_avg = 40; K = 1.; T = 1500
y_init = range(-40*0.2, 40*0.2, length = num_rays)
ys = y_init[findall(0 .≤ y_init .≤ 1.)]
xs = range(0,40., step = dt)
v_range = logrange(0.001, 1, length = 20)
a = 0.2; dot_radius = 0.2*0.25; softness = 0.2; θ_range = range(0,π/4, length = n_avg) 

for v0 in v_range
    dat = get_branch_number(v0, y_init, xs, num_rays, K; prefix = "fermi_br_lyap", 
                           a, dot_radius, softness, θ_range, force = true) 
    ...
    save(plotsdir(savename("plot_nb_",(v0, num_rays), ".pdf")),fig)
end
